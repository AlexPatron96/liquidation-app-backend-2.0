...
const getSearch = async (req, res) => {
	try {
		...
		const day_start = req.query.day_start;
		const data = {
			id_client,
			numFact,
			dateInit,
		...
		const processData = await billService.searchQuery(data);
		console.log(typeof day_start);
		const result = day_start
			? processData.filter(
					(data) =>
						parseInt(data.client.route_day.day_id) ===
						parseInt(day_start)
			  )
			: processData;
		if (result) {
...
const getRouteBill = async (req, res) => {
	try {
		...
		res.status(200).json({
			message: `Invoices available por la ruta del   `,
			findRoute,
		});
	} catch (error) {
		res.status(400).json({ error: error.message });
...
const createItem = async (req, res) => {
	try {
		...
		if (existence) {
			...
			res.status(400).json({
				message: `Ya existe un registro con ese numero de Documento. Clienteid: ${id_client} #${num_bill} , Balance:${balance} , Total:${total_bill} `,
			});
		} else {
			...
			if (factWhite) {
				...
				if (validator.test(num_bill)) {
					...
					res.status(400).json({
						message: "El numero de factura no cumple con la validacion",
					});
				}
			}
...
const getIdItem = async (req, res) => {
	try {
		...
		if (result) {
			res.status(200).json({
				message: "Invoice by id found and selected..",
				result,
			});
		} else {
			res.status(400).json({
...
const getIdItemForEch = async (req, res) => {
	try {
		...
		for (let i = 0; i < data.length; i++) {
			...
			if (!result) {
				isExistError = true;
				invoicePosition = `Busca en la posicion ${
					i + 1
				} de la lista de facturas a liquidar, con numero de factura # ${
					data[i].num_bill
				}`;
				break;
			}
